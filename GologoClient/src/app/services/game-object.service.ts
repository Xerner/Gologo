...
export class GameObjectService {
    ...
    filter<T extends GameObject>(type: Type<T>): T[] {
        return Array.from(this.gameObjects.values()).filter(gameObject => gameObject instanceof type) as T[];
    }

    // TODO: utilize this when switching between levels
    // currently, this is not used because there is only one level
    private resetPools() {
